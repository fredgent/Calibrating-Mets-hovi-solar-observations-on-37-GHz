...
\fag{We should define what we mean by the target set, why we need it, what is its use.}
...
Previous works, such as \cite{metsahovi40}, \skk{(We need to discuss this with
Metsähovi people and survey for more references)} have studied MRO solar maps
at the level of individual radio samples and collected statistics of those
above a fixed threshold intensity. These have not included the historical
contour plots of the category C. Also analysis based on manually collected
values has been made, such as \cite{KLJU12,KR12} and \cite{VSV87}. For the
first time, the whole 40 years of data is available in a form that can be
accessed in a fully automated means, allowing detailed and analysis in accurate
heliographic coordinates. This includes, among other applications, calculating
temporal averages of radio samples within a given region on the rotating
heliographic surface. Heliographic features can be distinguished based on their
actual separateness. 

Cycle 21 maps have previously been analysed with manual methods in
\cite{KLJU12,KR12} using undigitised maps. The calibration process has been
developed further.
...
\fag{Add here explanation of identifying dim and bright features - technique and relevance to cataloguing such features.}
...
%FAG added emphasis of what is new
Our time series now extends the previously published data to include the 
observations digitally recovered from the historical maps covering cycle 21.
There is an imbalance in the statistics available across the time span, firstly
due to the more sparse observations during earlier epochs, and also due to the 
seasonal variation in quantity and quality of the specimens.
%FAG moved from above
\fag{We should define what we mean by the target set, why we need it, what is its use.}
Each specimen map has weight $w_i$, which is set as to balance the target set. 
There are more 
observations starting from year $2015$, while only a few maps for year $1989$. 
%MJK Often we do just the opposite: if there are less data for a certain epoch, then the weight is lower because that time interval is more uncertain. Please expand on the logic here.
%FAG What is the purpose of the weighting - is it an issue of pixels for plotting 
%FAG purposes? Is it a visualisation issue to ensure the data is visible, averaging over so many days?
Thus we will put more weight on the old 
maps.

%FAG moved para break from below
Summer days typically have dozens of maps as the observational day is long, while on winter there are only a few maps per day.
%Moved para break up
We get more samples from the low heliographic latitudes, since a greater length
of the solar equator is visible, compared to high latitude arcs.
During summer, the solar north pole is visible to Earth, while we also get
observations from high altitudes.
During winter, we only observe the Sun at low altitude, and the solar south
pole is visible. 
Further investigations are needed to exclude any bias effects arising from the
...
%FAG added
Without making adjustments for limb brightening effects of the telescope, there is a lack of data about the poles.
When the lim brightening is considered (Section\,\ref{sect:limb}) for the 
modern specimens, for which the raw data is available, pole activity becomes
visible, as displayed in Figure\,\ref{butterfly_clear_corr}.
The data near the poles has more statistical noise, but the increased intensity
is consistent across multiple specimens, and consistent with previous 
observations of polar activity in radio by \citet{SCGVPS14}.
