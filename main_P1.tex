...
>>>>>>> 030af870b7408ac05424905bf7f941253dc8deac
  ...
  \begin{enumerate}[A]
    ...
    were available. Applying pattern recognition techniques presented in \cite{masterthesis}, the digital scans of the paper contour maps were converted into
    ...
    MJK The final map in solar coordinates is presented in~\Fig{typicalmap}{\bf{(c)}.
  \end{enumerate}

Each line in a digital solar map file contains the exact time and coordinates of a sample as well as the recorded intensity value. The 
coordinates are relative to the expected location of the center of the Sun on the sky. The intensity is measured on 
$\SI{37}{GHz}$ and contains the value both on a linear and a logarithmic output from the amplifier. In this paper, we 
restrict the study into the linear scale. However, the logarithmic scale avoids saturation at extreme intensity values 
during flare events and will be a target of further research.

Fig.~\ref{typicalmap} shows the original uncalibrated intensities of a typical solar map. We need to normalize the 
