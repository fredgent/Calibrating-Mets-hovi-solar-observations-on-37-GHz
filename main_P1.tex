...
Each line in a digital solar map file contains the time and coordinates of a sample as well as the recorded intensity value. The 
coordinates are relative to the expected location of the center of the Sun on the sky. The intensity is measured on 
$\SI{37}{GHz}$ and contains linear and logarithmic output from the amplifier as digital values. In this paper, we 
restrict the study into the linear scale. However, the logarithmic scale avoids saturation at extreme intensity values 
during flare events and will be a focus of further research.

Fig.~\ref{typicalmap}{\bf(a-c)} shows original uncalibrated intensities. We need to normalize the 
signal levels and also correct for any minor offsets in the sample coordinates in order to have a properly centered map. 
The purpose of normalization is to have zero intensity at the background and unit intensity for the QSL. The applicable 
methods are described in Section\,\ref{sect:disk}. For the contour maps up to year 1987, we trust the graphical markings 
and no calibration is needed.




  %The digital maps do not specify the location of the solar disk.
...
%------------------------------------------------------------------------------
  ...
  The additional quantities ($...$) are included for sample $s_i$ as part of the calibration process described below.
  ...
  better. We thus need to determine the exact center for each map. Also, since the signal levels $u_i$ are on arbitrary scale, we 
  need to normalize them. Normalization is done using statistical functions $f$ and $g$ to calculate references for background and QSL intensities:
  \eqnl{map_normalization1}{
  \s{u}{background} = f(S) \; \text{and} \; \s{u}{QSL} = g(S) \text{.}
  }
  We then perform a linear transformation to obtain a normalized value $v_i$ for each sample $u_i$:
  \eqnl{map_normalization2}{
  v_i := \frac{u_i - \s{u}{background})}{\s{u}{QSL} - \s{u}{background}} \; \text{with} \; s_i \in S \text{.}
  }
  The normalized intensity is then zero for a full background sample and unity for a QSL sample. Once the intensity 
  levels are normalized for map $S$, we can calculate the position $(x_S, y_S)$ and radius $r_S$ of the visible solar disk. The centering 
  is done using another statistical function $h$ which analyzes the normalized $v_i$ values:
  \eqnl{map_centering1}{
  \left( x_S, y_S, r_S \right) := h(S) \text{.}
  }
  We can then calculate properly centered coordinates for each sample $s_i$. We denote $r_{\astrosun}$ for the expected visual radius of the solar disk, in radians.
  \eqnl{map_centering2}{
  x^{\prime}_i := (x_i - x_S)\frac{r_{\astrosun}(t_{\mathrm{mid},S})}{r_S} \; \text{and} \; y^{\prime}_i := (y_i - y_S)\frac{r_{\astrosun}(t_{\mathrm{mid},S})}{r_S} \text{.}
  }
  We assume that the visual radius is constant during the observation of $S$. We use the middle time, $t_{\mathrm{mid},S}$, for evaluation:
  \eqnl{map_centering3}{
  t_{\mathrm{mid},S} = \frac{\operatorname{min} \left\{ t_i \right\} + \operatorname{max} \left\{ t_i \right\}}{2} \text{.}
  }
