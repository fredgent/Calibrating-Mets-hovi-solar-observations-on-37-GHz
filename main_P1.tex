...
\subsection{Data sources}\label{sect:source}
  ...
  Each sample observation yields an intensity map with respect to right 
  ascension, $\RA$, and declination, $\Dec$, coordinates.
  Each scan, denoted $S$ and spanning a couple of minutes, comprises a set of
  samples,  $s_i \in S$
  \eqnl{radio_sample}{
  s_i &=& 
  \left( t_i, \RA_i, \Dec_i, u_i \right) %\\ \nonumber
  %&=& \left( t(s_i), \delta \mathrm{RA}(s_i), \delta \mathrm{dec}(s_i), u(s_i) \right) \text{,}
  }
  where $t_i$ is the time at which the intensity $u_i$ is observed and 
  $\s{t}{min}(S) \le s_i \le \s{t}{max}(S)$.
  ...
  aspect ratio:
  \eqnl{relative_radec}{
  \RA_i  &=& \frac{\RA - \RA_{\astrosun}(t_i)}{\cos \left( \Dec_{\astrosun}(t_i) \right)} \\
  \Dec_i &=& \Dec - \Dec_{\astrosun}(t_i)\text{.}
  }
  We consider two disk methods to determine the solar disk position and size
  from each map:
  \begin{enumerate}
    \item intentionally weighting the center of mass of the radio sample
    coordinates.
    \item fitting a circle for those sample coordinates which we, by geometry,
    expect to belong to the solar limb.
  \end{enumerate}
  ...
  \begin{enumerate}[a]
    \item  sort the intensity samples and detect the point of zero curvature from the
    index vs. intensity dependency, the $S$-curve, 
    \item iteratively calculate the
    statistical mean and standard deviation of the intensity of 
    those points which are located within the solar disk and not too close
    to the limb
